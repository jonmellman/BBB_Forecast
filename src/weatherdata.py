...
class WeatherData(object):
    ...
    def __init__(self, zipCode='29613'):
        ...
        Constructor
        '''
        self.LOGTAG = os.path.basename(__file__)
        
        self.zipCode = zipCode
        utils.log(self.LOGTAG, 'Retrieving forecast for zip code ' + zipCode)
    ...
    def _chooseRequestDay(self):
        ...
        if datetime.datetime.now().hour >= WeatherData.requestDayCutoffHour:
            ...
            utils.log(self.LOGTAG, 'It\'s afternoon. Getting forecast for: ' + str(tomorrow))
            return tomorrow.day
        else:
            today = datetime.date.today()
            utils.log(self.LOGTAG, 'It\'s before noon. Getting forecast for: ' + str(today))
            return today.day
